...
const View: React.FC = () => {
  ...
  const categoryElements = categories?.map((category, index) => (
    <div
      ...
      className="rounded-sm bg-sky-100 p-2 text-center"
    >
      <Link
        to={`/stories?searchCategories=${encodeURIComponent(category.name)}`}
  ...
  return (
    <MainContainer>
      <div className="w-full text-slate-800">
        <div className="mb-8 flex w-full flex-col items-center rounded-sm bg-sky-100 p-2">
          <div>Henhouse</div>
          <div className="text-sm">
            A clucking-good online repository of fiction
          </div>
        </div>
        <div className="mb-2 flex w-full flex-col items-center rounded-sm bg-sky-100 p-2">
          Categories
        </div>
